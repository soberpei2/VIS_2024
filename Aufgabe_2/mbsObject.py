...
class mbsObject:
    ...
    def getActor(self, mbsObject):
        ...
        if isinstance(mbsObject, rigidBody):
            ...
            bodyActor.GetProperty().SetColor(mbsObject.parameter["color"]["value"][0] / 255,
                                             mbsObject.parameter["color"]["value"][1] / 255,
                                             mbsObject.parameter["color"]["value"][2] / 255)
...
class GenericTorque(mbsObject):
    ...
    def __init__(self, text):
        ...
        # Aufrufen des Mutterklassenkonstruktors (Ginge auch mit super, dann müsste man self nicht
        # übergeben)
        mbsObject.__init__(self, "Torque", "Rigid_EulerParameter_PAI", text, parameter)